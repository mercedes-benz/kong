...
function initialize_test_list() {
    ...
    if [ -z "$TESTS" ]
    then
        TESTS=$(gojira run -t $NEW_VERSION -- kong migrations status | jq -r '.new_migrations | .[] | (.namespace | gsub("[.]"; "/")) as $namespace | .migrations[] | "\($namespace)/\(.)_test.lua"')
